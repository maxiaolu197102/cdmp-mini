...
type userBatchPatchOperationExecutor struct {
	service *UserService
}

func (e *userBatchPatchOperationExecutor) Prepare(_ context.Context, env *operation.OperationEnvelope) error {
	if env == nil {
...
func (e *userBatchPatchOperationExecutor) Execute(ctx context.Context, env *operation.OperationEnvelope) (*operation.OperationResult, error) {
	...
	var update v1.User
	if err := stdjson.Unmarshal(env.Payload, &update); err != nil {
		decodeErr := fmt.Errorf("decode batch payload: %w", err)
		return &operation.OperationResult{OperationID: env.ID, State: operation.StateFailed, Fatal: true, Error: decodeErr}, decodeErr
	}

	if err := e.service.processUserBatchPatch(ctx, &update); err != nil {
		fatal := classifyBatchPatchError(err)
...
func (e *userBatchPatchOperationExecutor) Compensate(_ context.Context, env *operation.OperationEnvelope) (*operation.OperationResult, error) {
...
func classifyBatchPatchError(err error) bool {
	if err == nil {
		return false
	}
	switch errors.GetCode(err) {
	case code.ErrInvalidParameter:
		return true
	default:
		return false
	}
