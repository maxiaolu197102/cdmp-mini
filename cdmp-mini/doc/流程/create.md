...
3. 降级模式流程
   ...
   │  │  ├─ 否 → 返回字段冲突错误
   │  │  └─ 是 → 放行
   │  └─ 否 → 调用EnsurePlaceholder写入占位符（尽力而为）→ 放行
   └─ 结束
...
5. 无缓存流程
   ├─ 检查预检（preflight）是否命中
   │  ├─ 是 → 用预检结果判定所有者→允许则放行，否则返回冲突
   │  └─ 否 → 直接查库校验→有冲突返回错误，无冲突放行
   └─ 结束
...
7. 缓存未命中流程
   ...
   │  └─ 否 → 尝试SetNX写入占位符
   │     ├─ 占位失败（键已存在）→ 读取已有占位符
   │     │  ├─ 占位符存在 → 判断所有者是否允许放行→否返回冲突，是放行
   │     │  └─ 占位符不存在 → 重试SetNX
   │     └─ 占位成功 → 标记placeholderAcquired=true
   │        ├─ 检查缓存已预热且允许跳过查库→是→放行
   │        └─ 否 → 8. 查库流程
   └─ 结束
