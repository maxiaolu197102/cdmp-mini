...
## OperationPipeline 队列降级约定

在使用 nexuscore 提供的 OperationPipeline（无论是基于 Redis 还是其他后端）时，推荐遵循如下「队列降级」约定，以避免在服务端引入隐性、不易观测的 fallback 语义：

- **降级≠自动切换到内存队列**：
    - 一旦队列后端（如 Redis 集群）被判定为不可用或处于降级状态，不建议在服务端悄然切换到内存队列继续入队；
    - 原因是内存队列通常不具备跨实例持久化能力，容易在进程重启或扩缩容时造成「部分请求只存在于本地内存」的隐性数据丢失，且难以在监控中准确感知和重放。
- **推荐策略：「降级 = 拒绝入队，依赖调用方重试」**：
    - 上层业务在调用 `operationPipeline.Submit` 之前，应基于统一的健康视图（例如 RedisHealth / DegradeActive）判断队列后端是否可用；
    - 当检测到队列处于降级/不可用状态时：
        - 直接返回一个明确的业务错误（如 `ErrServerBusy` 或等价错误码），并配合清晰的错误文案（例如「队列暂不可用，请稍后重试」）；
        - 不执行 Submit，不向任何内存队列或临时结构写入任务；
        - 由调用方依赖幂等键 + 重试策略（如退避重试或补偿任务）来保证「不丢业务」。
- **正常态行为保持不变**：
    - 当队列后端健康时，依旧通过 `operationPipeline.Submit` 写入队列，然后由后台 Worker 或 inline 预跑通过 `ProcessOnce` 消费；
    - 失败重试、死信/补偿等机制仍按各业务自定义的策略实现，与「降级=拒绝入队」约定不冲突。

通过上述约定，可以在库层保持 OperationPipeline 本身语义简单、可预测：

- 队列后端是否可用由上层健康探测和降级策略决定；
- 降级时统一表现为「不入队 + 立即返回可观测错误」，而不是在库内部悄悄维护第二套队列语义；
- 业务侧只需围绕「Submit 可能失败，需要幂等 + 重试」这一稳定约定设计调用路径即可。
